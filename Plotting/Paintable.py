...
class StackPaintable(Paintable):
    ...
    def __init__(self, name, definition):
        ...
        self.uncertainty = ROOT.TH1F()
        self.legendData = []
        self.createPaintable()

    def createPaintable(self):
        self.uncertainty = self.getContribution(self.definition["Processes"][self.definition["Order"][0]]).Clone()
        for i_process in range(1, len(self.definition["Order"])):
            self.uncertainty.Add(self.getContribution(self.definition["Processes"][self.definition["Order"][i_process]]))
        self.uncertainty.SetFillStyle(3454)
        self.uncertainty.SetFillColor(ROOT.kBlue+2)
        self.uncertainty.SetLineColor(ROOT.kBlack)
        self.uncertainty.SetLineWidth(2)
        self.uncertainty.SetMarkerSize(0)
        self.uncertainty.SetLineColor(ROOT.kWhite)
        for i_bin in range(self.uncertainty.GetNbinsX()):
            err = self.uncertainty.GetBinError(i_bin)
            self.uncertainty.SetBinError(i_bin, err)
    ...
    def addToLegend(self, legend):
        ...
        legend.AddEntry(self.uncertainty, "Stat. unc", "f") # comment out if don't want Stat. unc in legend
    ...
    def draw(self, option = ""):
        ...
        ROOT.gStyle.SetErrorX(0.5) # comment out if don't want x error bar on data nor error band
        self.uncertainty.Draw("e2same") # comment out if don't want uncertainty error band on plot
...
class DataPaintable(Paintable):
    ...
    def addToLegend(self, legend):
        legend.AddEntry(self.datahist, "Data", "lep")
